...
const TAB_STORAGE_KEY = "devpilot-open-tabs";
...
export const useTabStore = create<TabStore>((set, get) => ({
  ...
  restoreTabs: async () => {
    try {
      ...
      // For DevPilot we don't validate against a server API —
      // session tabs are validated by checking if the session exists in chatStore
      ...
      const activeId =
        data.activeTabId && validTabs.some((t) => t.sessionId === data.activeTabId)
          ? data.activeTabId
