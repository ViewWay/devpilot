...
export function TabBar() {
  ...
  const handleClose = (sessionId: string) => {
    ...
    // Check if session is running (streaming)
    const isLoading = useChatStore.getState().isLoading;
    const streamingId = useChatStore.getState().streamingMessageId;
    const isActive = sessionId === useChatStore.getState().activeSessionId;
    const isRunning = isActive && isLoading && !!streamingId;
  ...
  return (
    ...
    >
      {canScrollLeft && (
        <button
          onClick={() => scroll("left")}
          className="flex-shrink-0 w-7 h-[37px] flex items-center justify-center text-[var(--color-text-tertiary)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-surface-hover)]"
        >
          <ChevronLeft size={16} />
        </button>
      )}
      ...
      {contextMenu && (
        ...
        >
          <button
            onClick={() => {
              handleClose(contextMenu.sessionId);
              setContextMenu(null);
            }}
      ...
      {closingTabId && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/30">
          <div
            className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-6 max-w-sm w-full mx-4"
  ...
  return (
    <div
      ...
      className={`
        ...
        ${
          isDragOver
            ? "before:absolute before:left-0 before:top-[6px] before:bottom-[6px] before:w-[2px] before:bg-[var(--color-brand)] before:rounded-full"
            : ""
        }
      `}
    ...
    >
      ...
      {tab.type === "settings" && (
        <Settings size={14} className="flex-shrink-0 text-[var(--color-text-tertiary)]" />
      )}
      {tab.type === "scheduled" && (
        <Clock size={14} className="flex-shrink-0 text-[var(--color-text-tertiary)]" />
      )}
